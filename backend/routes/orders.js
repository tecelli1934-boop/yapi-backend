...
const orderValidation = [
  body('items').isArray({ min: 1 }).withMessage('Sipariş sepeti boş olamaz'),
  body('items.*.id').notEmpty().withMessage('Ürün ID geçersiz'),
  body('items.*.quantity').isInt({ min: 1 }).withMessage('Ürün adedi en az 1 olmalıdır'),
  body('items.*.price').isFloat({ min: 0 }).withMessage('Ürün fiyatı negatif olamaz'),
